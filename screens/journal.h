...
//
// Inherits Screen; uses JournalEntry as its data model and JournalStorage for
// file persistence. Demonstrates:
...
private:
    ...
    QPushButton *m_deleteBtn;   // delete currently selected saved entry

    QTimer      *m_timer;       // 1-second tick → updateClock()
    JournalStorage m_storage;   // file I/O service for journal entries
};
